...
cultureBoard = [
    [0 for _ in range(MAX_CONTAINER)] for _ in range(MAX_CONTAINER)
]
...
def isOutOfBounds(row, col):
    return not (0 <= row < containerSize and 0 <= col < containerSize)
...
def arrangeMicroorganisms(
    microId, injectionRow1, injectionCol1, injectionRow2, injectionCol2
):
    for row in range(containerSize):
        for col in range(containerSize):
...
def relocateMicroorganisms(microCount):
    ...
    for row in range(containerSize):
        for col in range(containerSize):
            ...
            microBoundingStart[currentMicroId] = (
                min(microBoundingStart[currentMicroId][0], row),
                min(microBoundingStart[currentMicroId][1], col),
            )
            microBoundingEnd[currentMicroId] = (
                max(microBoundingEnd[currentMicroId][0], row),
                max(microBoundingEnd[currentMicroId][1], col),
            )
            ...
            for newCol in range(containerSize - clusterColCount + 1):
                ...
                for dr in range(clusterRowCount):
                    for dc in range(clusterColCount):
                        ...
                        if (
                            cultureBoard[originalRow][originalCol]
                            != currentMicroId
                        ):
                            continue
